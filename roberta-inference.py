...
def token_wise_scoring(sequences, label_ids, attention_mask, tokenizer, device, model):
    """
        Scores the sequences token-wise using the language model.
        ...
        Args:
            ...
            model (RobertaForMaskedLM): The language model.

        Returns:
            int: The predicted label index.
    """
...
def prepare_input(sequences, label_ids, pad_token_id):
    """
        Prepares the input sequences by padding them and creating attention masks.
        ...
        Returns:
            tuple: Contains padded sequences, label IDs, and attention masks.
    """
...
def score_task(question, choices, tokenizer, device, model):
    """
        Scores a task using the language model.
...
def get_lm_score(model, batch, label_ids, attention_mask):
    ...
    Args:
        model (RobertaForMaskedLM): The language model.
        batch (tensor): Tensor containing the batch of tokenized sequences.
        label_ids (tensor): Tensor containing the label IDs.
        attention_mask (tensor): Tensor containing the attention masks.

    Returns:
        numpy.ndarray: Array of loss values.
    """
...
def init_model(model_name: str,
    ...
    """
       ...
       Returns:
           tuple: Contains the model and tokenizer.
    """
